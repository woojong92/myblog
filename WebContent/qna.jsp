...
<html lang="en">
  ...
  <body>
	...
	<div class="container">
			...
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd">
				...
				<tbody>
					...
					<%
						...
						ArrayList<QnA> list = QnA_DAO.getList(pageNumber);
						for(int i=0; i< list.size(); i++){
						
					%>
					<tr>
						<td><%= list.get(i).getBbsID() %></td>
						<td><a href="qna_view.jsp?bbsID=<%=list.get(i).getBbsID()%>"> <%= list.get(i).getBbsTitle().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></a></td>
						<td><%= list.get(i).getBbsUserID().replaceAll("<","&lt").replaceAll(">","&lt").replaceAll(" ","&nbsp").replaceAll("\n","<br>") %></td>
		...
		if(pageNumber != 1) {
		%>
		<a href="qna.jsp?pageNumber=<%=pageNumber-1 %>" class="btn btn-success btn-arraw-left">이전</a>
		<%
		} 
		if(QnA_DAO.nextPage(pageNumber+1)) {
		%>
		<a href="qna.jsp?pageNumber=<%=pageNumber+1 %>" class="btn btn-success btn-arraw-left">다음</a>
		<%
		}
		%>
