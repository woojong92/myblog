...
<html lang="en">
  ...
  <body>
	...
	<%
		...
		free_board_DAO free_board = new free_board_DAO();	
	%>
		...
		<table class="table table-hover">
			<tbody>
				<%
					...
					free_board list = free_board.getBbs(bbsID);
				%>
				<tr>
					<td> 글번호 </td>
					<td><%= list.getBbsID() %></td>
				...
				<tr>
					<td> 첨부파일 </td>
					<td> <a href="free_board_fileDown.jsp?bbsID=<%=list.getBbsID()%>">  <%= list.getBbsFileName() %> </a></td>
				</tr>
				<tr>
					<td> 유효성 확인 </td>
	...
	<%
		String BbsUserID=list.getBbsUserID();
		if(UID.equals(BbsUserID)){
	%>
		<a href="free_board_update.jsp?bbsID=<%=list.getBbsID() %>" class="btn btn-primary">수정</a>
